...
#include "movement_pkg/nodes/center_goal_slow_action.h"


BT::CenterGoalSlow::CenterGoalSlow(std::string name) 
...
BT::CenterGoalSlow::~CenterGoalSlow() {}

void BT::CenterGoalSlow::WaitForTick()
{
    while (ros::ok())
    {
        ROS_TAGGED_ONCE_LOG("WAIT FOR TICK", "DEFAULT", false, "Wait_CenterGoalSlow");
        tick_engine.Wait();
        ROS_TAGGED_ONCE_LOG("TICK RECEIVED", "DEFAULT", false, "Received_CenterGoalSlow");
        ...
        while (get_status() == BT::IDLE)
        {
            ...
            if (fabs(xerror_) < 5 && fabs(yerror_) < 5)
            {
            ...
            if (xerror_ >= 11 || xerror_ <= -11)
            {
                ...
                ROS_COLORED_LOG("Centering head in X=%f", VIOLET, false, angle_mov_x_);
            ...
            else if (yerror_ >= 20 || yerror_ <= -20)
            {
                ...
                ROS_COLORED_LOG("Centering head in y=%f", VIOLET, false, angle_mov_y_);
...
void BT::CenterGoalSlow::writeHeadJoint(double ang_value, bool is_pan)
{
...
double BT::CenterGoalSlow::calculateDistance(double head_tilt)
{
    double distance = CAMERA_HEIGHT_ * tan(M_PI * 0.5 + head_tilt - hip_pitch_offset_);
    return distance;
}

void BT::CenterGoalSlow::Halt()
{
    set_status(BT::HALTED);
    ROS_COLORED_LOG("CenterGoalSlow HALTED: Stopped centering ball", ORANGE, false);
}
