...
#include "movement_pkg/nodes/right_search_goal_action.h"


BT::RightSearchGoal::RightSearchGoal(std::string name) 
: ActionNode::ActionNode(name)
{
    ...
    thread_ = std::thread(&RightSearchGoal::WaitForTick, this);
}

BT::RightSearchGoal::~RightSearchGoal() {}

void BT::RightSearchGoal::WaitForTick()
{
    while (ros::ok())
    {
...
void BT::RightSearchGoal::writeHeadJoint(double ang_valueX)
{
...
void BT::RightSearchGoal::Halt()
{
    set_status(BT::HALTED);
    ROS_TAGGED_ONCE_LOG("SearchGoal HALTED: Stopped right search goal", "ORANGE", false, "Halted_rightsearchgoal");
}
