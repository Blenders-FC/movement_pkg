...
#include "movement_pkg/nodes/center_ball_YOLO_CPU_action.h"


BT::CenterBallYOLOCPU::CenterBallYOLOCPU(std::string name) 
: ActionNode::ActionNode(name)
{
    ...
    thread_ = std::thread(&CenterBallYOLOCPU::WaitForTick, this);
}

BT::CenterBallYOLOCPU::~CenterBallYOLOCPU() {}

void BT::CenterBallYOLOCPU::WaitForTick()
{
    while (ros::ok())
    {
        ROS_TAGGED_ONCE_LOG("WAIT FOR TICK", "DEFAULT", false, "Wait_CenterBallYOLOCPU");
        tick_engine.Wait();
        ROS_TAGGED_ONCE_LOG("TICK RECEIVED", "DEFAULT", false, "Received_CenterBallYOLOCPU");
...
void BT::CenterBallYOLOCPU::writeHeadJoint(double ang_value, bool is_pan)
{
...
void BT::CenterBallYOLOCPU::Halt()
{
    ...
    ROS_COLORED_LOG("CenterBallYOLOCPU HALTED: Stopped centering ball", ORANGE, false);
}
