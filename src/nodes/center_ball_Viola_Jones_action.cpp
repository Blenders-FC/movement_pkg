...
#include "movement_pkg/nodes/center_ball_Viola_Jones_action.h"


BT::CenterBallViolaJones::CenterBallViolaJones(std::string name) 
: ActionNode::ActionNode(name)
{
    ...
    thread_ = std::thread(&CenterBallViolaJones::WaitForTick, this);
}

BT::CenterBallViolaJones::~CenterBallViolaJones() {}

void BT::CenterBallViolaJones::WaitForTick()
{
    while (ros::ok())
    {
        ROS_TAGGED_ONCE_LOG("WAIT FOR TICK", "DEFAULT", false, "Wait_CenterBallViolaJones");
        tick_engine.Wait();
        ROS_TAGGED_ONCE_LOG("TICK RECEIVED", "DEFAULT", false, "Received_CenterBallViolaJones");
...
void BT::CenterBallViolaJones::Halt()
{
    set_status(BT::HALTED);
    ROS_COLORED_LOG("CenterBallViolaJones HALTED: Stopped centering ball", ORANGE, false);
}
