...
#include <movement_pkg/nodes/left_long_kick_action.h>

BT::LeftLongKick::LeftLongKick(std::string name) : ActionNode::ActionNode(name), utils()
{
    // Publisher
    write_joint_pub_ = nh.advertise<sensor_msgs::JointState>("/robotis_" + std::to_string(robot_id) + "/set_joint_states", 0);

    type_ = BT::ACTION_NODE;
    thread_ = std::thread(&LeftLongKick::WaitForTick, this);
}

BT::LeftLongKick::~LeftLongKick() {}

void BT::LeftLongKick::WaitForTick()
{
    while(ros::ok())
    {
        ...
        while (get_status() == BT::IDLE)
        {
            ...
            ROS_COLORED_LOG("Left long kicking!", TEAL, false);
            //node loop
...
void BT::LeftLongKick::Halt()
{
    set_status(BT::HALTED);
    ROS_TAGGED_ONCE_LOG("LeftLongKick HALTED: Stopped long kick", "ORANGE", false, "Halted_long_kick");
}