...
public:
    // Constructor
    explicit FOVCalculation(float ball_x, float ball_y, double head_pan, double head_tilt);
    // Virtual destructor for inheritance
    virtual ~FOVCalculation();

protected:
    // External functions
    double calcFocalLength_y();
    double calcFocalLength_x();
    void calcPanAngle();
...
private:
    ...
    const double CAM_HEIGHT_DEF_RES = 480;    // pixels
    const double CAM_WIDTH_DEF_RES = 640;     // pixels
    const double H_CENTER_DEF_RES = 240;      // CAM_HEIGHT / 2
    const double W_CENTER_DEF_RES = 320;      // CAM_WIDTH / 2
    ...
    const double CAM_HEIGHT_ORIG_RES = 1080;     // pixels
    const double CAM_WIDTH_ORIG_RES = 1920;      // pixels
    const double H_CENTER_ORIG_RES = 540;        // CAM_HEIGHT / 2
    const double W_CENTER_ORIG_RES = 960;        // CAM_WIDTH / 2
    ...
    const double CAM_HEIGHT_SMALL_RES = 240;       // pixels
    const double CAM_WIDTH_SMALL_RES = 426;        // pixels
    const double H_CENTER_ORIG_RES = 120;          // CAM_HEIGHT / 2
    const double W_CENTER_ORIG_RES = 213;          // CAM_WIDTH / 2
